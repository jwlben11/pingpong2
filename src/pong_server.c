...
/*** TO BE DONE START ***/
	...
	if (udp_socket < 0) {
		if (errno != EADDRINUSE) 
			fail_errno("UDP Pong could not create the socket");
		continue; // Prova la prossima porta
	}

	// Imposta l'indirizzo e la porta per il binding
	gai_rv = getaddrinfo(NULL, port_number_as_str, &gai_hints, &pong_addrinfo);
	if (gai_rv != 0) {
		fail_errno("getaddrinfo failed");
	}

	// Tentativo di binding del socket
	bind_rv = bind(udp_socket, pong_addrinfo->ai_addr, pong_addrinfo->ai_addrlen);
	freeaddrinfo(pong_addrinfo); // Libera la memoria allocata da getaddrinfo
	if (bind_rv < 0) {
		if (errno != EADDRINUSE) {
			fail_errno("UDP Pong could not bind the socket");
		}
		close(udp_socket); // Chiudi il socket se il binding fallisce
		continue; // Prova la prossima porta
	}

	// Se il binding ha successo, memorizza il numero di porta
...
/*** TO BE DONE START ***/
    ...
    if (server_socket < 0) {
        fail_errno("Pong Server could not create the socket");
    }

    // Ottieni le informazioni sull'indirizzo del server
    gai_rv = getaddrinfo(NULL, argv[1], &gai_hints, &server_addrinfo);
    if (gai_rv != 0) {
        fail("getaddrinfo failed: %s");
    }

    // Binding del socket all'indirizzo e alla porta
    if (bind(server_socket, server_addrinfo->ai_addr, server_addrinfo->ai_addrlen) < 0) {
        fail_errno("Pong Server could not bind the socket");
    }

    // Metti il socket in ascolto per le connessioni in entrata
    if (listen(server_socket, SOMAXCONN) < 0) {
        fail_errno("Pong Server could not listen on the socket");
    }
