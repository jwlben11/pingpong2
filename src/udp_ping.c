...
/*** TO BE DONE START ***/
	...
	if (sent_bytes < 0) {
		fail_errno("UDP ping could not send message");
	}
...
/*** TO BE DONE START ***/
	...
	gai_hints.ai_flags = AI_PASSIVE;      // Allow wildcard IP address
...
/*** TO BE DONE START ***/
	...
	if (connect(ping_socket, (struct sockaddr*)pong_addrinfo->ai_addr, pong_addrinfo->ai_addrlen) == -1) {
    perror("Error connecting UDP socket");
    fail_errno("Error connecting UDP socket");
	}
...
/*** TO BE DONE START ***/
    ...
    gai_hints.ai_protocol = IPPROTO_TCP;  // TCP protocol
...
/*** TO BE DONE START ***/
    ...
    if (connect(ask_socket, server_addrinfo->ai_addr, server_addrinfo->ai_addrlen) < 0) {
        fail_errno("Error connecting to server");
    }
...
/*** TO BE DONE START ***/
	...
	if (strncmp(answer, "OK", 2) != 0)
        fail("Received unexpected response from server: %s\n");
